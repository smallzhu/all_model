        ...
        for phase in ['train', 'valid']:
            ...
            for inputs, labels in tqdm(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase=='train'):
                    ...
                    loss = criterion(output, labels, features)
            ...
            if phase == 'valid':
                if epoch_acc > best_valid_acc:
                    ...
                    torch.save(model.state_dict(),os.path.join(save_root, '{}_acc{}.pth'.format(time.strftime("%m_%d_%H_%M", time.localtime()),int(best_valid_acc*10000))))
            ...
            with open(os.path.join(save_root, "record.csv").format(args.model), 'a+', newline='')as f:
...
if __name__ == '__main__':
    ...
    num_classes = 3
    lr = 0.00001
    save_root = "/45TB/dc/parking/paper_checkpoint/{}/{}".format(args.save_name, args.model)
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    record_headers = ['epoch', 'phase', 'epoch_loss', 'epoch_acc', 'local_time']
    with open(os.path.join(save_root, "record.csv"),'a+',newline='')as f:
        f_csv = csv.DictWriter(f, record_headers)
        f_csv.writeheader()
    ...
    trans = transforms.Compose([
        transforms.RandomChoice(
            [transforms.Resize(args.resize),
             transforms.RandomResizedCrop(args.resize, scale=(0.6, 1))]
        ),
        ...
        transforms.RandomApply(
            [transforms.RandomAffine(degrees=20, translate=(0.1,0.1), shear=(20,20,20,20))], p=0.3),  # 以0.2的概率应用旋转
        # transforms.RandomApply(
    ...
    parking_datasets = {x: dataloader.dataset(img_height=args.resize[0], img_width=args.resize[1], images_path=data_dir, num_classes=num_classes, train_valid=x, trans=trans)
    ...
    model.load_state_dict(torch.load('/45TB/dc/parking/paper_checkpoint/IBloss/vit_base_patch16_384/11_25_15_18_acc9707.pth'))

    logging.info("model loaded")

    model = model.to(devcie)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)
    adjust_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.1)
    # criterion = SmoothCrossEntropy()
    cls_num_list = [15861, 12071, 3266]
    per_cls_weights = 1.0 / np.array(cls_num_list)
    per_cls_weights = per_cls_weights / np.sum(per_cls_weights) * len(cls_num_list)
    per_cls_weights = torch.FloatTensor(per_cls_weights).cuda()

    criterion = loss.IB_FocalLoss(weight=per_cls_weights, alpha=10000)
