        ...
        for phase in ['train', 'valid']:
            ...
            if phase == 'valid':
                if epoch_acc > best_valid_acc:
                    ...
                    torch.save(model.state_dict(),os.path.join(save_root, '{}_{}_acc{}.pth'.format(args.save_model_name,time.strftime("%m_%d_%H_%M", time.localtime()),int(best_valid_acc*10000))))
...
if __name__ == '__main__':
    ...
    batch_size = 10
    lr = 0.00001
    save_root = "/45TB/dc/parking/paper_checkpoint/{}/{}".format(args.model,args.save_model_name)
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    record_headers = ['epoch', 'phase', 'epoch_loss', 'epoch_acc', 'local_time']
    with open(os.path.join(save_root, args.record+".csv"),'a+',newline='')as f:
        f_csv = csv.DictWriter(f, record_headers)
        f_csv.writeheader()
    ...
    print(model)
    # model.load_state_dict(torch.load('/45TB/dc/parking/paper_checkpoint/vit_base_patch16_384/lstm_vit/lstm_vit_11_16_11_00_acc9176.pth'))
