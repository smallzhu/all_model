...
def valid(model):
    ...
    for inputs, labels in bar:
        ...
        x = torch.sum(preds == labels.data)
        correct += x
        running_corrects.update(int(x), args.batch_size)

        bar.set_description(f"{running_corrects.avg}")
    print("correct {}".format(correct// dataset_sizes))
...
#######temp
    ...
    parking_datasets = dataloader.dataset(img_height=args.resize, img_width=args.resize, images_path=data_dir, num_classes=args.num_classes, train_valid="valid", trans=0)
