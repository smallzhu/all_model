        ...
        for phase in ['train', 'valid']:
            ...
            if phase == 'valid':
                if epoch_acc > best_valid_acc:
                    ...
                    torch.save(model.state_dict(),os.path.join(save_root, 'b_loss_{}_acc{}.pth'.format(time.strftime("%m_%d_%H_%M", time.localtime()),int(best_valid_acc*10000))))
            ...
            with open(os.path.join(save_root, "b_record.csv").format(args.model), 'a+', newline='')as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize', default=(570, 336))
    ...
    batch_size = 10
    lr = 0.00001
    save_root = "/21TB/dc/parking/checkpoint/{}".format(args.model)
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    record_headers = ['epoch', 'phase', 'epoch_loss', 'epoch_acc', 'local_time']
    with open(os.path.join(save_root, "b_record.csv"),'a+',newline='')as f:
        f_csv = csv.DictWriter(f, record_headers)
        f_csv.writeheader()
    ...
    parking_datasets = {x: dataloader.dataset(img_height=args.resize[0], img_width=args.resize[1], images_path=data_dir, num_classes=args.num_classes, train_valid=x, trans=trans)
    ...
    model.load_state_dict(torch.load('/home/dc/pytorch_image_model/checkpoint/vit_large_patch16_224/b_loss_10_23_07_24_acc9506.pth'))
    ...
    criterion = loss.binary_cross_entropy.DenseBinaryCrossEntropy()
